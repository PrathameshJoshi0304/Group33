...
<%
	try	
	{
		...
		<tr><th>ID</th><th>Name</th><th>Email ID</th></tr>
		<%
		while(rs.next())
		{
			int Acceptid=rs.getInt(1);
			%>
			<tr><td><%=rs.getInt(1)%></td><td><%=rs.getString(2)%></td><td><%=rs.getString(4)%></td><td><input type="button" class="btn btn-success" id="btnAccept" onclick="getAcceptId(<%=Acceptid%>);" value="Accept"/></td><td><input type="button" class="btn btn-danger" id="btnReject" onclick="getRejectId(<%=Acceptid%>);" value="Reject"/></td></tr>
			<%
		}
	}
	catch(Exception e)
